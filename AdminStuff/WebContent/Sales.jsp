...
if (date==null || date.isEmpty() || date.equals("")) {
	response.sendRedirect("WelcomeAdmin.jsp?try=No date entered");
	}
date= date+"/01";
...
try {
	...
	String str2 = "	SELECT COUNT(Buys.CancelledDate) AS cancelled FROM Buys, Tickets WHERE Buys.BuysTicketNumber = Tickets.TicketNumber AND Buys.Class Like 'E%' = ? AND Buys.CancelledDate IS NOT NULL AND  YEAR(Buys.CancelledDate) = YEAR(?) AND MONTH(Buys.CancelledDate) = MONTH(?)";
	...
	{	
		...
		response.sendRedirect("WelcomeAdmin.jsp?try= Invalid Date. Try again");
	con.close();
	result.close();
	stmt.close();
